...
import { Quote, Pencil, Trash2, Music, Loader2, Video, ShieldCheck, AlertCircle } from 'lucide-react';
...
export const MemoryCard = forwardRef<HTMLDivElement, MemoryCardProps>(({ memory, index, onEdit, onDelete }, ref) => {
  ...
  const renderSignatureOverlay = (label = "Deploy for Full Media") => (
    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/40 backdrop-blur-[2px] p-6 text-center">
      <div className="bg-white/90 dark:bg-zinc-900/90 rounded-full px-4 py-2 flex items-center gap-2 border border-peach/30 shadow-xl mb-4">
        {hasError ? <AlertCircle className="w-4 h-4 text-amber-500" /> : <ShieldCheck className="w-4 h-4 text-peach" />}
        <span className="text-[10px] font-bold uppercase tracking-widest text-foreground">
          {hasError ? "Media Link Fragile" : label}
        </span>
      </div>
      <p className="text-white text-xs md:text-sm font-serif italic max-w-xs drop-shadow-md">
        {hasError 
          ? "The primary link has faded, but the essence remains preserved in our signature."
  ...
  return (
    ...
    >
      <div className="absolute top-6 right-8 opacity-0 group-hover:opacity-100 transition-all duration-300 flex gap-2 z-40">
        ...
        <Button variant="ghost" size="icon" className="h-10 w-10 rounded-full bg-red-50/90 dark:bg-red-950/90 border border-red-100/50 text-red-500 hover:bg-red-500 hover:text-white shadow-sm" onClick={() => { if (window.confirm("Remove from our eternal archive?")) onDelete?.(); }}><Trash2 className="w-4 h-4" /></Button>
      </div>
      ...
      {(memory.type === 'video' || memory.type === 'image') && (
        <div className="space-y-8">
          <div className="relative overflow-hidden rounded-[2rem] aspect-video w-full border-4 border-warm-paper dark:border-zinc-900 shadow-inner" style={{ backgroundColor: fallbackColor }}>
            ...
            {(isSandbox || hasError) && memory.previewUrl ? (
              <>
                ...
                {renderSignatureOverlay(isSandbox ? "Deploy for Full Media" : "Media Unavailable")}
              </>
            ) : memory.type === 'video' ? (
              <video
                src={memory.mediaUrl}
            ...
            {hasError && !memory.previewUrl && (
              <div className="absolute inset-0 z-20 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm">
                <div className="flex flex-col items-center gap-2">
                  <Video className="w-12 h-12 text-peach/40" />
                  <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Signal Lost</span>
                </div>
              </div>
            )}
          </div>
          <p className="text-xl md:text-3xl font-serif italic text-center text-foreground/90 px-4 leading-relaxed">"{memory.content}"</p>
        </div>
      )}
      {memory.type === 'audio' && (
        <div className="space-y-10">
          ...
          <div className="relative p-8 rounded-[2.5rem] border border-peach/10 flex flex-col items-center gap-6 overflow-hidden bg-warm-paper/50 dark:bg-zinc-900/50">
            ...
            {memory.mediaUrl && !isSandbox && !hasError ? (
              <audio 
                src={memory.mediaUrl} 
                controls 
                className="w-full relative z-10 opacity-90 h-10 filter dark:invert" 
                preload="metadata" 
                onError={handleMediaError}
              />
            ) : (
              <div className="w-full h-12 bg-white/60 dark:bg-black/60 rounded-full border border-peach/20 relative z-10 flex items-center justify-center gap-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                  {hasError ? "Audio Link Fragile" : "Audio Signature Verified"}
                </span>
              </div>
            )}
          </div>
